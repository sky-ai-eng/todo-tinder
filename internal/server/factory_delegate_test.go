...
func TestHandleFactoryDelegate_DelegateErrorPreservesClaim(t *testing.T) {
	s := newTestServer(t)
	s.SetSpawner(delegate.NewSpawner(s.db, s.prompts, nil, s.chains, s.tasks, s.agentRuns, s.entities, s.reviews, s.pendingPRs, s.events, s.taskMemory, sqlitestore.New(s.db).RunWorktrees, sqlitestore.New(s.db).Orgs, sqlitestore.New(s.db).Tx, nil, websocket.NewHub(), "haiku", ""))
...
func TestHandleFactoryDelegate_RefusedWhenBotDisabled(t *testing.T) {
	s := newTestServer(t)
	s.SetSpawner(delegate.NewSpawner(s.db, s.prompts, nil, nil, s.tasks, s.agentRuns, s.entities, s.reviews, s.pendingPRs, s.events, s.taskMemory, sqlitestore.New(s.db).RunWorktrees, sqlitestore.New(s.db).Orgs, sqlitestore.New(s.db).Tx, nil, websocket.NewHub(), "haiku", ""))
